...
let fullName = [];
// writing an anon function
runners.forEach(runner => {
  fullName.push(`${runner.first_name} ${runner.last_name}`)
})

// wwriting a callback function
let derrickCallBack = runner => {
  fullName.push(`${runner.first_name} ${runner.last_name}`)}
runners.forEach(derrickCallBack);
...
let result = runner => {
  largeShirts.push(runner.filter(shirt_size === "L"))}
runners.forEach(result);

// runners.forEach(runner =>{
  // largeShirts.push(`${runner.shirt_size}`.filter("L"))
// })
